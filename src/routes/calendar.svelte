...
<script>
	...
	/** @type {Record<number, import('scripts/types').Event[]>}*/
	const monthWiseEvents = {};
	for (let i = 0; i < 12; i++) monthWiseEvents[i] = [];

	for (let event of events) {
		const month = new Date(event.datetime).getMonth();
		monthWiseEvents[month].push(event);
	}
</script>
